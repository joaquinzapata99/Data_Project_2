...
def obtener_info_contacto(match_id):
    ...
    SELECT 
        ...
        v.Telefono as Voluntario_Telefono,
        m.Voluntario_Necesidad
    FROM `data-project-2-449815.dataflow_matches.match` m
    LEFT JOIN `data-project-2-449815.dataflow_matches.solicitudes` s
    ON m.Solicitud_ID = s.ID
    LEFT JOIN `data-project-2-449815.dataflow_matches.voluntarios` v
    ON m.Voluntario_ID = v.ID
